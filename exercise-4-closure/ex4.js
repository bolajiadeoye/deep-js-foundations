/* 
1. A feature has been been partially added to this application, but you need to complete it:
...
2. A new function `setupWorkDescription(..)` has been added, with a `// TODO` comment marking where your code should go. Using the same click handler pattern found in `initUI()`, add a click handler that:
...
3. **BONUS:** There are at least two other ways to implement this feature without closure. Experiment with alternate implementations. What are the pros/cons of these 
*/
...
function setupWorkDescription(workEntryData,$workDescription) {
	let description = workEntryData.description;
	$workDescription.text(formatWorkDescription(description));

	if (workEntryData.description.length > maxVisibleWorkDescriptionLength) {
		$workDescription.addClass("shortened");
		$workDescription.on('click', expandworkDescription);
		/*$workDescription.on('click', function(evt){
			expandworkDescriptionNoClosure($workDescription, workEntryData.description);
		});*/
	}

	function expandworkDescription(){
		$workDescription
		.removeClass("shortened")
		.text(description)
		.off('click', expandworkDescription);
	}
}

function expandworkDescriptionNoClosure(descriptionEl, description){
	descriptionEl
	.removeClass("shortened")
	.text(description)
	.off('click', expandworkDescriptionNoClosure);
}
